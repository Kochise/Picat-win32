...
void expandStackNullifyUntaggedCellsTrail(void)
{
...
hunt_for_corrupted_sf_chain(void) {
	BPLONG_PTR	ptr = sfreg;
...
hunt_for_corrupted_susp_var(void) {
	BPLONG_PTR	addr;
	BPLONG_PTR	ptr = stack_low_addr;
...
check_topunset_ar_subgoal(void) {
...
in_ref_chain(BPLONG term, BPLONG_PTR ptr)
{
