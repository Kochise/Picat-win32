...
BPLONG table_area_notin_use(void){
...
int table_area_num_expansions(void){
	return ta_record_ptr->num_expansions;
}

int c_INITIALIZE_TABLE(void){
...
int c_table_reset_subgoal_ar(void){
	BPLONG_PTR sp;
...
int c_TABLE_GET_ONE_ANSWER(void){
	BPLONG Call, t1, t2;
...
int c_TABLE_GET_ALL_ANSWERS(void){
	BPLONG Call, Answers;
...
*/
int c_table_cardinality_limit(void){
...
int c_set_all_table_cardinality_limit(void){
	BPLONG card;
	BPLONG i;
	SYM_REC_PTR sym_ptr;
	BPLONG_PTR ep;
...
void reset_temp_complete_subgoal_entries(void){
	BPLONG i;
	BPLONG_PTR subgoal_entry, ptr;
...
void printSubgoalTableEntry(BPLONG_PTR ptr)
{
	BPLONG arity;
...
void printAnswer(BPLONG_PTR ptr, BPLONG arity)
{
	BPLONG i;
	BPLONG term;
	void myWriteTerm();
...
void init_picat_table_maps(void){
	BPLONG i;
...
BPLONG table_maps_buckets_size(void){
	BPLONG i, size;
	MAP_RECORD_PTR map_ptr;
