...
/* c+a1*x1+...+an*xn = 0 */
int nary_interval_consistent_eq(register BPLONG n)
{
...
/* c+x1+...+xn = 0 */
int nary_interval_consistent_nocoe(register BPLONG n)
{
...
int nary_interval_consistent_ge(register BPLONG n)
{
	register	BPLONG_PTR	dv_ptr, var_ptr, coe_ptr;
	register	BPLONG		i, first, last;
	register	BPLONG		a, x;
...
*/
int c_REDUCE_DOMAINS_IC_EQ(void) {
	BPLONG		Terms, Const;
...
/* Terms=[(An, Vn),...,(A1, V1)].
	Reduce domains of Vi's such that the constraint An*Vn+..., A1*V1+Const>=0 is interval consistent
*/
int c_REDUCE_DOMAINS_IC_GE(void) {
	BPLONG		Terms, Const;
	BPLONG_PTR	old_arreg;
	int			n, res;
