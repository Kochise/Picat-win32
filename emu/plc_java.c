...
static void java2prolog(JNIEnv *env, jobject val, BPLONG arg) {
	...
	if ((*env)->IsInstanceOf(env, val, class_Number)) {
		jobject obj = (*env)->CallObjectMethod(env, val,
												...
												0);
		if (obj) str = (char *)((*env)->GetStringUTFChars(env, obj, NULL));
...
/*		bprolog_string_2_term(str, arg);  */
		...
		str = (char *)((*env)->GetStringUTFChars(env, val, NULL));
		PuStr(arg, str);
/*		PuAtom(arg, str); */
...
static jref addr2ref(JNIEnv *env, BPLONG addr) {
...
static void initialize(JNIEnv *env, jref arrayobj) {
