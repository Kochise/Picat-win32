...
int b_STREAM_CHECK_CURRENT_TEXT_INPUT(void) {
...
int b_STREAM_CHECK_CURRENT_TEXT_OUTPUT(void) {
...
/**********************************************************************/
void bp_trim_trailing_zeros(char *bp_buf)
{
	BPLONG	len;
...
void bp_write_insert_dot_zero_if_needed(char *bp_buf)
{
...
int c_format_get_line_pos(void) {
	BPLONG	op;
...
int c_format_retrieve_codes(void) {
...
int c_FORMAT_PRINT_INTEGER(void) {
...
int b_NAME0_cf(BPLONG op1, BPLONG op2)	/* op2 is made to be the string of the name of op1 */
{
	...
	if (ISATOM(op1)) {
		...
		sprintf(bp_buf, "%.15f", floatval(op1));
		bp_trim_trailing_zeros(bp_buf);
		return string2codes(bp_buf, op2);
	} else {
		exception = illegal_arguments; return BP_ERROR;
...
int print_term_to_buf(BPLONG term) {
	...
	if (ISREF(term)) {
		...
		if (IS_SUSP_VAR(term)) {
			...
			sprintf(bp_buf, "%.15f", floatval(term));
			bp_trim_trailing_zeros(bp_buf);
...
void picat_str_to_c_str(BPLONG lst, char *buf, BPLONG buf_size) {
	...
	while (ISLIST(lst)) {
		...
		len = GET_LENGTH(sym_ptr);
		if (i + len >= buf_size) {
			printf("hreg = " BPULONG_FMT_STR " local_top = " BPULONG_FMT_STR " buf_size = " BPLONG_FMT_STR " \n", (BPULONG)heap_top, (BPULONG)local_top, buf_size);
...
int c_PICAT_FORMAT_TO_STRING_ccff(void) {
