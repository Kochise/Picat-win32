...
/**/
BPLONG picat_build_var(void){
	NEW_HEAP_FREE;
	return (BPLONG)(heap_top-1);
}

BPLONG bp_build_var(void){
...
BPLONG bp_build_list(void)
{
	return picat_build_list();
}

BPLONG picat_build_nil(void)
{
	return nil_sym;
}

BPLONG bp_build_nil(void)
{
	return nil_sym;
}
...
void initialize_bp_str_bag(void){
	char *mallo();
...
int c_CallBpFromC(void){
	BPLONG goal;
	goal = ARG(1, 1);
	return bp_call_term(goal);
}

int c_START_TRACE(void){

	trace_toam = 1;
	return 1;
}

int c_END_TRACE(void){
	trace_toam = 0;
	return 1;
}
