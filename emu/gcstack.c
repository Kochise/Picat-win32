...
void gc_initialize_ar_chain(void){
	BPLONG_PTR cp, f;
	int n, b_initialized;
...
*/
int gc_globalize_sf_chain(void){
	BPLONG_PTR f;
	BPLONG mask_size;
...
void mark_stack_references_ar_chain(void){
	BPLONG_PTR f;
	f = arreg;
...
void mark_stack_references_sf_chain(void){
	BPLONG_PTR f;
	f = sfreg;
	while (f<breg){
		if (!FRAME_IS_DEAD(f)){
...
BPLONG_PTR gcMoveAliveFramesOutSf(void)
{
...
BPLONG_PTR gcMoveAliveFramesOutAr(void)
{
	BPLONG_PTR f, next;
...
int b_ENABLE_GC(void){
	bp_gc = old_bp_gc;
	return 1;
}

int b_DISABLE_GC(void){
	old_bp_gc = bp_gc;
	bp_gc = 0;
	return 1;
}

int c_GET_GC_TIME(void){
